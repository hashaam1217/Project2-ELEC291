...
void USART2_Handler(void) 
{
    ...
    if (USART2->ISR & BIT7) // is it a TXE interrupt?
        usart_tx2();
    if (USART2->ISR & BIT5 && setup) // is it an RXNE interrupt?
        usart_rx2();
    if (USART2->ISR & BIT5 && (setup==0)) // is it an RXNE interrupt?
        usart_rx2_int();
}
...
void usart_tx2 (void)
{
    ...
    if (GetBufCount2(&ComTXBuffer2)&&setup) USART2->TDR=GetBuf2(&ComTXBuffer2);
    else
    {
