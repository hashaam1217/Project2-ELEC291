...
void main (void)
{
	...
	while(1)
	{
		...
		printf ("V@P2.2=%7.5fV, V@P2.3=%7.5fV\r", joy_data[0], joy_data[1]);
		...
		if(P3_7==0)
		{
			...
			sprintf(joy_x, "%.2f", joy_data[0]);
			sprintf(joy_y, "%.2f", joy_data[1]);
			
			// concatenate strings
			remote_data[0] = '\0';
			
			strcpy(remote_data, joy_x);
		        strcat(remote_data, ", ");
		    	strcat(remote_data, joy_y);
			
			sprintf(buff, "concatenated string: %d\r\n", remote_data);
			sendstr1(buff);
			putchar('.');
			waitms_or_RI1(200);
		}
