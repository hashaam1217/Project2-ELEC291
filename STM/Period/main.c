...
void main(void)
{
	...
	while(1)
	{
		...
		if(count>0)
		{
			...
			f=1.0/T;
            f = (-1/1858.69)*f+101.809;
            if (f < 10)
            {
                sprintf(buff, "C=%.2f nF", f);
                LCDprint(buff, 1, 1);
                sprintf(buff, "Black capacitor");
                LCDprint(buff, 2, 1);
            }
            else
            {
                sprintf(buff, "C=%.4f uF", f/1000);
                LCDprint(buff, 1, 1);
                sprintf(buff, "Yellow capacitor");
                LCDprint(buff, 2, 1);
            }
